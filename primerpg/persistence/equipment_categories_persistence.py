...
select_equipment_categories_query = "SELECT * FROM %s WHERE unique_id = ?" % equipment_categories_table
...
def populate_equipment_categories_table():
    ...
    if not should_reload_from_file(data["dependencies"], file_name, equipment_categories_table):
        return
