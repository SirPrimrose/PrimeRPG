...
insert_player_tasks_query = "INSERT INTO %s (player_id, task_id, time_started) VALUES (?, ?, ?)" % player_tasks_table
delete_player_task_query = "DELETE from %s WHERE player_id = ? AND task_id = ?" % player_tasks_table
...
def get_player_task(unique_id: int) -> PlayerTaskCore:
    ...
    return init_player_task(result)


def insert_player_task(task: PlayerTaskCore):
