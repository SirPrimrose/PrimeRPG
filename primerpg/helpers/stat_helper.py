...
from primerpg.data_cache import get_equipment_stat
...
def get_total_scaled_stat_value(stat_id, skills: List[EntitySkill], equipment: List[EntityEquipment]) -> float:
    total_value = 0.0
    for e in equipment:
...
def get_scaled_stat_value_for_item(item_id: int, stat_id: int, skills: List[EntitySkill]) -> float:
